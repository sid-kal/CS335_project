...
{
    // returns st entry from local table or global table if it is declared as "global x"
    if(entries.count(s))
...
// st_entry* symbol_table::inherit_tree_lookup(string s, vector<string> type)
// {
//     symbol_table* st_ptr=this;
//     while(st_ptr)
//     {
//         st_entry* temp=st_ptr->is_present(s, type);
//         if(temp)
//             return temp;
