...
funcdef : DEF NAME {
                    ...
                    $1->temp_count=temporary_counter;
...
parameters ARROW_test_or_not COLON {
                ...
                curr_st->size_of_params=curr_st->offset;
...
suite {
                int temp_cnt_temp=temporary_counter;

                tac[$1->begin_func_idx].op1=to_string(curr_st->offset-$1->size_of_params);
                tac[$1->begin_func_idx].op2=to_string(curr_st->offset+(temp_cnt_temp-$1->temp_count)*8-$1->size_of_params);
                ...
                if(!is_return_type_present)
                {
                    tac.push_back(quad("leave", "", "", "", LEAVE));
...
expr_stmt   : testlist_star_expr annassign {
                        ...
                        )
                        {
                            ...
                            st_entry* temp_st_en=st_stack->tables[st_stack->tables.size()-2]->inherit_tree_lookup(st.substr(5));
                            if(temp_st_en)
                            {
                        ...
                        if($1 && $1->name == "NAME")
                        {
                            if($2->rvalue.size())
                            {
                                ...
                                tac.back().target_entry=new_st_entry;
                            }
                        }
                        else
                        {
            ...
            | testlist_star_expr augassign testlist {
                        ...
                        if($1->name=="NAME")
                        {
                            temp=st_stack->lookup($1->lexval);
                        }
                        gen_augassign_tac($1,$2,$3, temp);
            ...
            | testlist_star_expr EQUAL_testlist_star_expr_kleene {
                        ...
                        if($2)
                        {
                            ...
                            {
                                tac.push_back(quad("=", $2->rvalue, "", $1->lexval, NAME_ASSIGNMENT));
                                tac.back().target_entry=st_stack->lookup($1->lexval);
                            }
                            else
                            {
...
EQUAL_testlist_star_expr_kleene :EQUAL testlist_star_expr EQUAL_testlist_star_expr_kleene  {
                        ...
                        else
                        {
                            ...
                            if($2 && $2->name == "NAME")
                            {
                                ...
                                tac.back().target_entry=st_stack->lookup($2->lexval);
                            }
                            else
                            {
...
for_stmt    : FOR exprlist IN testlist COLON {curr_nesting_depth++; inside_loop=true;
                        ...
                        if($2->name=="NAME")
                        {
                            ...
                            tac.back().target_entry=st_stack->lookup($2->lvalue);
                        }
                        else
                        ...
                        if($2->name=="NAME")
                        {
                            ...
                            tac.back().op1_entry=st_stack->lookup($2->lvalue);
                        }
                        else
...
comparison  : expr {                            
            ...
            | comparison comp_op expr {
                ...
                if($1->info->type=="str"  && $3->info->type=="str")
                {
                    map<string, string> mp={{"<=", "le"}, {">=", "ge"}, {"<", "l"}, {">", "g"}, {"==", "e"}, {"!=", "ne"}};
                    tac.push_back(quad("save registers", "", "", "", SAVE_REG));
                    ...
                    tac.push_back(quad("call", "strcmp"+mp[$2->lexval], "2","", FUNCCALL));
                    string new_temp=new_temporary();
                    tac.push_back(quad("=","return_value","",new_temp,RET_VAL));
                    $$->rvalue = new_temp;
                    tac.push_back(quad("stackpointer", "+16", "","", STACK_MANIPULATION));
                    tac.push_back(quad("restore registers", "", "", "", RESTORE_REG));
                }
                else
                {
                    if(str1=="float")
...
factor  : PLUS_or_MINUS_or_BITWISE_NOT factor {
                ...
                //     tac.push_back(quad($1->lexval,"",$2->rvalue, $2->rvalue, ARITH));
                    tac.push_back(quad($1->lexval,"",$2->rvalue, $2->rvalue, UNARY));
            ...
            | atom_expr trailer  {
                ...
                if(!inside_type_hint)
                {
                    ...
                    else if($1->info->type[0]=='$')
                    {
                        ...
                        tac.push_back(quad("save registers", "", "", "", SAVE_REG));
                        // if(size)
                            tac.push_back(quad("stackpointer", "-"+to_string(8), "","", STACK_MANIPULATION));
...
atom    : LEFT_PAREN  test RIGHT_PAREN {
        ...
        | LEFT_BRACKET testlist_comp_or_not RIGHT_BRACKET {
                        ...
                        tac.push_back(quad("save registers", "", "", "", SAVE_REG));
                        tac.push_back(quad("stackpointer", "-8", "","", STACK_MANIPULATION));
                        tac.push_back(quad("param", new_temp, "", "", PUSHPARAM));
                        tac.push_back(quad("call", "allocmem", "1", "", FUNCCALL));
                        tac.push_back(quad("=","return_value","",new_temp,RET_VAL));
                        $$->rvalue=new_temp;
        ...
        | STRING_plus {
                    ...
                    string new_temp=new_temporary();
                    string new_string=".str"+to_string(str_cnt);
                    str_cnt++;
                    global_strings.push_back("\""+$1->lexval+"\"");
                    tac.push_back(quad("=", new_string,"" ,new_temp, NAME_ASSIGNMENT));
                    $$->rvalue=new_temp;
...
void print_help(){
    ...
    cout<<"-output_tac <filename>: specifies output file to store the Three Address Code "<<endl;
    cout<<"-output_x86 <filename>: specifies output file to store the x86 assembly file "<<endl;
...
int main(int argc, char *argv[])
{
    ...
    for(int i=1;i<argc;i++)
    {
        ...
        else if((string)argv[i]=="-output_tac")
        {
            if(i+1>=argc)
            {
        ...
        else if((string)argv[i]=="-output_x86")
        {
            if(i+1>=argc)
            {
                cerr<<"Error: Invalid parameters - output file missing"<<endl;
    ...
    generate_x86(tac, output_filename2);
    return 0;
}
